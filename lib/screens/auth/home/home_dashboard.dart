...
class HomeDashboard extends StatefulWidget {
  ...
  const HomeDashboard({super.key, required this.userName, required this.userId, this.themeMode = ThemeMode.system, this.onThemeModeChanged = _ignoreThemeChange});
...
class _HomeDashboardState extends State<HomeDashboard> {
  ...
  void _onNavTap(int index) {
    ...
    if (index == 4) _open(ProfileScreen(userId: widget.userId, themeMode: widget.themeMode, onThemeModeChanged: widget.onThemeModeChanged));
  }
  ...
  Widget _sidebar() => Container(width: 240, color: Theme.of(context).colorScheme.surface, padding: const EdgeInsets.fromLTRB(20, 28, 20, 20), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
    _brand(), const SizedBox(height: 42), Text('WORKSPACE', style: TextStyle(fontSize: 11, letterSpacing: 1.2, fontWeight: FontWeight.w800, color: Theme.of(context).colorScheme.onSurfaceVariant)), const SizedBox(height: 12),
    ...
    Text('Stay in sync, one day at a time.', style: TextStyle(color: Theme.of(context).colorScheme.onSurfaceVariant, height: 1.4)),
  ]));
  ...
  Widget _sideItem(IconData icon, String label, int index) { final selected = _selectedIndex == index; return Padding(padding: const EdgeInsets.only(bottom: 8), child: ListTile(
    ...
    leading: Icon(icon, color: selected ? AppColors.primary : Theme.of(context).colorScheme.onSurfaceVariant),
    title: Text(label, style: TextStyle(fontWeight: selected ? FontWeight.w800 : FontWeight.w600, color: selected ? AppColors.primary : Theme.of(context).colorScheme.onSurface)), onTap: () => _onNavTap(index),
  )); }
  ...
  Widget _stats() {
    ...
    return Row(children: items.map((item) => Expanded(child: Padding(
      ...
      child: Container(padding: const EdgeInsets.all(14), decoration: _card(), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Text(item.$1, style: TextStyle(fontSize: 11, color: Theme.of(context).colorScheme.onSurfaceVariant, fontWeight: FontWeight.w600)),
      ])),
    ))).toList());
  }
  ...
  Widget _taskRow(String title, String subject, String due, Color color, bool done) => Container(key: ValueKey('overview-task-$title'), margin: const EdgeInsets.only(top: 10), padding: const EdgeInsets.all(15), decoration: _card(), child: Row(children: [Icon(done ? Icons.check_circle : Icons.radio_button_unchecked, color: done ? AppColors.success : AppColors.lightTextSecondary), const SizedBox(width: 12), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(title.trim(), style: TextStyle(fontWeight: FontWeight.w800, decoration: done ? TextDecoration.lineThrough : null)), const SizedBox(height: 5), Text('$subject  •  $due', style: TextStyle(fontSize: 12, color: color, fontWeight: FontWeight.w700))])), const Icon(Icons.more_horiz, color: AppColors.lightTextSecondary)]));
        ...
        : SizedBox(height: 112, child: ListView.separated(
            ...
            itemBuilder: (context, index) {
              ...
              return Container(width: 220, padding: const EdgeInsets.all(16), decoration: _card(), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                Text('${subject['subject_code'] ?? 'Course'}  •  3 tasks remaining', style: TextStyle(fontSize: 11, color: Theme.of(context).colorScheme.onSurfaceVariant)),
              ]));
            },
          )),
  ]);
  ...
  BoxDecoration _card() => BoxDecoration(color: Theme.of(context).colorScheme.surface, borderRadius: BorderRadius.circular(18), border: Border.all(color: Theme.of(context).colorScheme.outlineVariant), boxShadow: [BoxShadow(color: AppColors.primary.withOpacity(.06), blurRadius: 12, offset: const Offset(0, 4))]);
  Widget _bottomNav() => BottomAppBar(height: 72, color: Theme.of(context).colorScheme.surface, child: Row(mainAxisAlignment: MainAxisAlignment.spaceAround, children: [_navItem(Icons.home_rounded, 'Home', 0), _navItem(Icons.check_circle_outline, 'Tasks', 1), FloatingActionButton(onPressed: _showAddMenu, backgroundColor: AppColors.primary, foregroundColor: Colors.white, child: const Icon(Icons.add)), _navItem(Icons.calendar_month_outlined, 'Schedule', 3), _navItem(Icons.person_outline, 'Profile', 4)]));
  Widget _navItem(IconData icon, String label, int index) => IconButton(tooltip: label, onPressed: () => _onNavTap(index), color: _selectedIndex == index ? AppColors.primary : Theme.of(context).colorScheme.onSurfaceVariant, icon: Column(mainAxisSize: MainAxisSize.min, children: [Icon(icon, size: 23), Text(label, style: TextStyle(fontSize: 10, color: Theme.of(context).colorScheme.onSurfaceVariant))]));
}
